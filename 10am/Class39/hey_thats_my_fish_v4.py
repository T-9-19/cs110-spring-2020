...
class HeyThatsMyFish:
    ...
    def get_heading(self, delta_row, delta_col):
        """ Return a heading (in row/col direction) for checking individual tiles. """

        heading_row = 0
        if delta_row != 0:
            heading_row = delta_row // abs(delta_row)

        heading_col = 0
        if delta_col != 0:
            heading_col = delta_col // abs(delta_col)

        if heading_row == 0:
            heading_col *= 2

        return (heading_row, heading_col)
    ...
    def valid_move(self, tile):
        ...
        (heading_row, heading_col) = self.get_heading(delta_row, delta_col)

        if abs(delta_row) != abs(delta_col) and delta_row != 0:
            return False

        row_index = current_row + heading_row
        col_index = current_col + heading_col
        while row_index != target_row or col_index != target_col:
            if self._tiles[row_index][col_index].has_penguin() or \
               not self._tiles[row_index][col_index].is_present():
               return False
            row_index += heading_row
            col_index += heading_col
