...
class Car(EventHandler):
    ...
    def move(self, direction):
        """ Move rectangle to the right/left 20 pixels."""
        (x, y) = self._center
...
class Button(EventHandler):
    ...
    def handle_mouse_press(self, event):
        """ Change the button color and move the car when pressed. """
        self._circle.set_fill_color("green")
        self._car.move(self._direction)

    def handle_mouse_release(self, event):
        """ Change the button color back when releasing mouse buttion. """
        self._circle.set_fill_color("lightgray")
